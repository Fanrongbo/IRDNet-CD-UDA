...
def CAS(self):
    ...
    source_samples = samples['Img_source']  # list

    source_sample_paths = samples['Path_source']
    source_nums = [len(paths) for paths in source_sample_paths]
    # print('source_sample_paths',source_nums,source_sample_paths)
    ...
    # print('target_sample_paths',target_nums,target_sample_paths)
    ...
    # print('aaa',len(source_samples), source_nums, len(target_samples), target_nums)#10 [3, 3, 3, 3, 3, 3, 3, 3, 3, 3] 10 [3, 3, 3, 3, 3, 3, 3, 3, 3, 3]
    return source_samples, source_nums, target_samples, target_nums


class CDD(object):
    def __init__(self, num_layers, kernel_num, kernel_mul,
                 num_classes, intra_only=False,Device=None, **kwargs):
        self.device=Device
    ...
    def split_classwise(self, dist, nums):
        ...
        # print('num_classes',num_classes,dist.shape)
        start = end = 0
        dist_list = []
        for c in range(num_classes):
            start = end
            end = start + nums[c]
            # print(c,start,end,dist)
            dist_c = dist[start:end, start:end]  # [30, 30]
            dist_list += [dist_c]
        return dist_list
    ...
    def compute_paired_dist(self, A, B):
        ...
        B_expand = B.unsqueeze(0).expand(bs_A, bs_T, feat_len)  # [30, 30, 31]
    ...
    def forward(self, source, target, nums_S, nums_T):
        ...
        for i in range(self.num_layers):  ##feat  probs

            cur_source = source[i]  # [30, 2048],[30, 31] 30=3*10=batch*self.opt.TRAIN.NUM_SELECTED_CLASSES
            cur_target = target[i]  # [30, 31]
            # print('iii', i,cur_source.shape )
            dist = {}
            dist['ss'] = self.compute_paired_dist(cur_source, cur_source)  # [30, 30]
            dist['tt'] = self.compute_paired_dist(cur_target, cur_target)
            dist['st'] = self.compute_paired_dist(cur_source, cur_target)
            # print('cur_source', dist['ss'].shape,dist['st'].shape)
        ...
        if not self.intra_only:
            ...
            inter_mask = ((torch.ones([num_classes, num_classes]) \
                                  - torch.eye(num_classes)).type(torch.ByteTensor)).to(self.device)
